...
const { createAlert, createAction, getKbStatus } = require('./kb')
...
async function run({ config, minutes }, [ suiteId ]) {
  ...
  const queues = deployments.map(async deployment => {
    ...
    return await runQueue(alertNames, CONCURRENT_ALERT_CREATION, async (alertName) => {
      try {
        return await createAlert(deployment.kbUrl, alertName, '.kibana-event-log*', [{
          id: createdActionId,
          group: 'threshold met',
          params: {
            documents: [{
