...
async function getEventLog(deployment, esUrl, batchSize = BATCH_SIZE) {
  const docs = await retry(RETRY_ATTEMPTS, RETRY_SECONDS, `getting event log for ${deployment}`, async () =>
