...
async function createDeployment({ config, stack, name, deploymentName, esSize, kbSize, tmMaxWorkers, tmPollInterval }) {
  ...
  payload.resources.elasticsearch = payload.resources.elasticsearch.map(removeUnsupportedESConfig);
...
/** @type { (elasticsearch: { plan: { cluster_topology: { elasticsearch: { node_attributes? : { data: string; } } }[] } } ) => any } */
function removeUnsupportedESConfig(elasticsearch){
  elasticsearch.plan.cluster_topology = elasticsearch.plan.cluster_topology.filter((es) => {
    return !(es.elasticsearch.node_attributes && (es.elasticsearch.node_attributes.data == 'cold' || es.elasticsearch.node_attributes.data == 'warm'));
  })
