...
async function test() {
  ...
  const output = await runQueue(input, 2, fn)
  ...
  async function fn(n, worker) {
    console.log(`${worker}: ${new Date().toISOString()} starting ${n}`)
    await delay(100 * n)
    console.log(`${worker}: ${new Date().toISOString()} finished ${n}`)
    return n * n
  }
}
