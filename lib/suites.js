...
function validateSuite({ id, description, scenarios }) {
  ...
  for (const scenario of scenarios) {
    const { name, version, esSpec, kbSpec, ruleCount, ruleInterval } = scenario
    ...
    if (typeof ruleCount !== 'number') throw new Error(`${prefix} invalid scenario.ruleCount: "${ruleCount}"`)
    if (typeof ruleInterval !== 'string') throw new Error(`${prefix} invalid scenario.ruleInterval: "${ruleInterval}"`)
