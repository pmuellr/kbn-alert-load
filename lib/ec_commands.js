...
async function createDeployment({ config, name, deploymentName, esSize, kbSize }) {
...
/** @type { (key: string, result: any) => {endpoint: string, port: number, healthy: boolean, status: string} } */
function getResourceInfo(key, showDeployment) {
  ...
  return { endpoint, port, healthy, status }
}
