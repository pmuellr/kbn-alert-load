...
/** @type { (runName: string, template: string | null, suite: Suite, deployments: Deployment[], eventLog: EventLogRecord[], kbStatus: any[], esStatus: any[], kbTaskManagerStatusList: Map<string, Map<string, TaskManagerStats[]>>) => void } */
function generateReport(runName, template, suite, deployments, eventLog, kbStatus, esStatus, kbTaskManagerStatusList) {
  ...
  const content = readTemplate()
    ...
    .replace(/%template%/g, template || 'default')
    ...
    scenarios[scenarioName] = Array.from(tmStatsInScenario.entries())
      .reduce((summary, [tmId, tmStats]) => {    
        ...
        summary[randomName] = tmStats.map(tmStat => {
          const hasRuleStats = tmStat.stats.runtime.value.execution.duration["alerting:.index-threshold"]
